...
### M-4 — Rule 4: all unlocked equal auto-evens

| | |
|---|---|
| **Steps** | Add Jane 20% + Bob 20% + John 20% (all unlocked). Tap wizard. |
| **Expected** | All three set to 33.33% (or 33.34% for last). No popup. Total = 100%. |
| **Result** | **Fail** — wizard button still visible, padlocks locked, submit disabled at 99.99%. |
| **Root cause** | Three issues identified and fixed in this session: (1) typing auto-locked beneficiaries, routing to Rule 1 instead of Rule 4; (2) `applyWizardRounding` silently dropped remainder when all beneficiaries were locked; (3) validation tolerance was `< 0.01` not `<= 0.01`. |
| **Action** | **Retest.** All three root causes have been patched. Should now pass. |

### K-1 — Single 0% triggers dialog
...
### D-3 — Invalid percentage total blocked
...
### I-1 — Summary shows all investments
